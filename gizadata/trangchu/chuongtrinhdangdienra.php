...
{
    unset($GLOBALS['training_section_link'], $GLOBALS['training_section_link_text'], $GLOBALS['training_section_hide_button']);
}
...
if (isset($args)) {
    ...
    $custom_link = $GLOBALS['training_section_link'] ?? null;
    $custom_link_text = $GLOBALS['training_section_link_text'] ?? null;
    $section_content = get_training_section_content($custom_link, $custom_link_text);

    if (!empty($GLOBALS['training_section_hide_button'])) {
...
// ====== Lấy dữ liệu từ ACF Relationship ======
$training_posts = get_field('chuong_trinh_dao_tao_dang_dien_ra'); // array các post object
$training_programs = [];

if (!empty($training_posts) && is_array($training_posts)) {
    foreach ($training_posts as $post) {
        $training_programs[] = [
            ...
            'date'          => get_field('ngay', $post->ID) ?: get_the_date('d M Y', $post->ID),
            'time'          => get_field('time', $post->ID) ?: '',
            'countdown'     => get_field('countdown_text', $post->ID) ?: '',
            'location'      => get_field('vitri', $post->ID) ?: '',
            'googleMap'     => get_field('googlemap', $post->ID) ?: '',
            'language'      => get_field('language', $post->ID) ?: '',
            'badge'         => get_field('event_badge', $post->ID) ?: '',
    ...
    <div class="container">
        ...
        <?php if (empty($section_content['hide_button'])): ?>
            <div class="btn-see-more text-center mb-4">
                <a href="<?= esc_url($section_content['link']); ?>"><?= esc_html($section_content['link_text']); ?></a>
            </div>
        <?php endif; ?>
    </div>
        ...
        <div class="swiper-wrapper">
            <?php foreach ($training_programs as $index => $program): ?>
                <div class="swiper-slide">
                    <div class="ct<?= $index + 1; ?> relative card-event">
                        <div class="card-event_img">
                            <img src="<?= esc_url($program['image']); ?>" alt="<?= esc_attr($program['title']); ?>">
                        </div>
                        <div class="card-event_content">
                            <div class="card-event_wrapper">
                                ...
                                <div class="card-event_title"><?= esc_html($program['title']); ?></div>
                                <div class="card-event_time">
                                    <div class="a">
                                        ...
                                        <?= esc_html($program['date']); ?> | <?= esc_html($program['time']); ?>
                                    </div>
                                    <?php if (!empty($program['countdown'])): ?>
                                        <div class="card-event_countdown"><?= esc_html($program['countdown']); ?></div>
                                    <?php endif; ?>
                                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        new Swiper('.training-programs-swiper', {
            slidesPerView: 1,
            spaceBetween: 0,
            grabCursor: true,
            loop: true,
