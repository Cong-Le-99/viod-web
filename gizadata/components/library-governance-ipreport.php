<?php
$parent_category = get_category_by_slug('van-ban-phap-luat');
$child_category = get_category_by_slug('tu-sach-qtct-va-cac-an-pham');

// Query posts từ category con
...
if ($publications_query->have_posts()) {
    while ($publications_query->have_posts()) {
        ...
        if (!$image_url) {
            $image_url = get_template_directory_uri() . '/images/default.png'; // Default image
        }
    ...
    <div class="container">
        <div class="row">
            <div class="col-12 col-md-6 ">
                ...
                <button class="btn-register mt-5" onclick="window.location.href='/van-ban-phap-luat-lien-quan-3/?category=tu-sach-qtct-va-cac-an-pham'">
                Xem tất cả
                </button>
            </div>
            <div class="col-12 col-md-6 d-none d-md-block text-center">
                <img class="library-governance-img publications-and-research-img " src="<?php echo $publications[0]['image']; ?>" alt="Tài liệu và nghiên cứu"> 
            </div>
        </div>
    </div>
</section>
