...
// Lấy dữ liệu 2 mảng: đang diễn ra (A) và sắp diễn ra (B)
    ...
    foreach ($training_programs as $program) {
        if (is_object($program) && isset($program->ID)) {
            ...
            $item = [
                ...
                'time' => get_field('time', $program->ID) ?: get_field('gio', $program->ID) ?: '',
                    ...
                    <div class="swiper-slide training-program-item" data-index="<?php echo $index; ?>">
                        <div class="ct<?php echo $index + 1; ?> relative card-event w-100">
                            <div class="card-event_img">
                                <?php $imgSrc = !empty($program['image']) ? $program['image'] : $default_image; ?>
                                <img src="<?php echo $imgSrc; ?>" alt="<?php echo htmlspecialchars($program['title'] ?? ''); ?>">
                            </div>
